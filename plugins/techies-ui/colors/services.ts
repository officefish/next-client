...
const toHSL = (color: string | RGB) => {
    ...
    return `${convert(hsl[HUE])} ${convert(hsl[SATURATION])}% ${convert(hsl[LIGHTNESS])}% `
}
...
const generateForegroundColorFrom = (colorCode: string | RGB, percentage = 0.8) => {
...
const findKey = (rule: string) => {
    return (names.hasOwnProperty(rule)) 
        ? names[rule as NamesKeyType]
        : undefined
} 
...
export const injectThemes = (addBase:any, config:any, themes:any) => {
    ...
    const hasTechiesConfig = config('techies') !== undefined
                && config('techies.themes') !== undefined

    const useDefaultThemes = config('techies') !== undefined
                && config('techies.inbuilt') === true

    // define exports
    let includedThemes: {[k: string]: any} = {}
    let themeOrder:any[] = []
