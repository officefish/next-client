...
import useOnlyWithUser from "@/hooks/with-user-only"
...
const ProfileSettings: FC = (props: Props) => {
    ...
    const {user, mutate} = useOnlyWithUser()

    const [settingsMode, setSettingsMode] = useState<ESettingsMode>(ESettingsMode.BasicInfo)

    const handleInputMode = (e:MouseEvent<HTMLButtonElement> & {target: HTMLButtonElement}) => {
        e.preventDefault()
        const { target } = e
        switch(+target.value) {
            case 0: setSettingsMode(ESettingsMode.BasicInfo); break
            case 1: setSettingsMode(ESettingsMode.Status); break
            case 2: setSettingsMode(ESettingsMode.Visuals); break
            case 3: setSettingsMode(ESettingsMode.StylePreferences); break
            case 4: setSettingsMode(ESettingsMode.Security); break
            case 5: setSettingsMode(ESettingsMode.Integrations); break
            default: setSettingsMode(ESettingsMode.BasicInfo); break
        }
    }
        ...
        <div className="relative">
            ...
            <ProfileLayout avatar={avatar}>
                ...
                <Tabs>
                    ...
                    handler={handleInputMode}>Basic Info</TabItem>
                     <TabItem 
                    currentMode={settingsMode} mode={ESettingsMode.Status} 
                    handler={handleInputMode}>Status</TabItem>  
                    <TabItem 
                    currentMode={settingsMode} mode={ESettingsMode.Visuals} 
                    handler={handleInputMode}>Visuals</TabItem> 
                </Tabs>
...
interface ITabItem extends PropsWithChildren{
    ...
    handler: (e: MouseEvent<HTMLButtonElement, globalThis.MouseEvent> & { target: HTMLButtonElement }) => void
}
