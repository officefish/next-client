import { FC, useState, useCallback, useEffect, ChangeEvent, MouseEvent, useRef } from 'react'
import Cropper, {Area, Size} from 'react-easy-crop'

import getCroppedImg from '@/services/crop-image.service'
...
const ImageCropper:FC<IImageCropper> = ({image, aspect, onCrop}) => {
    ...
    useEffect(() => {
        ...
        if (cropContainer) {
            //@ts-ignore
            setCropContainerSize({width:cropContainer.clientHeight, height:cropContainer.clientHeight})
        }    
    }, [crop, zoom])
    ...
    return (
        <>
        <div ref={cropContainerRef} className='absolute top-12 left-0 bottom-[15%] right-0'>
                <Cropper 
                    image={image}
                    crop={crop}
                    zoom={zoom}
        ...
        <div className='absolute right-0 bottom-0 flex items-center gap-4 w-full'>
            <input type="range" min={0.5} max={3.0} step={0.05} value={zoom} className="range range-accent" onChange={onZoomChange} />
            <button className='btn btn-accent btn-outline btn-wide' onClick={onCropHandle}>Crop</button>
        </div>
        </>
    )
}
